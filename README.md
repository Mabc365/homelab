...
- **Multiple Host Profiles**: Support for different server roles, such as an AI host, a CasaOS host, or a full lab setup.
- **Docker and Docker Compose**: Easily run containerized applications.
- **CasaOS**: Lightweight home-server dashboard for managing apps and storage.
- **Portainer**: Web UI for managing Docker containers, images, networks, and volumes.
- **Nextcloud**: Self-hosted file sync, sharing, and collaboration.
- **DaVinci Resolve Project Server**: Shared PostgreSQL-backed project database for collaborative editing.
...
2. **Customize the `cloud-init/user-data.example` file**:
   - Replace `mabc365` with your desired username.
